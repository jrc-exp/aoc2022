...
stub_day:
	...
	@echo "Making day $(DAY)"
	@$(eval DAY_FILE := aoc/y2022/day$(DAY).py)
	@$(eval TEST_DAY_FILE := test/test_day$(DAY).py)
	@python -m aoc.y2022.get_test $(DAY)
	@cp -n aoc/y2022/day0.py $(DAY_FILE)
	...
	@if grep -q day$(DAY) setup.cfg; then echo exists; \
		else echo "  run_day$(DAY) = aoc.y2022.day$(DAY):main" >> setup.cfg && \
